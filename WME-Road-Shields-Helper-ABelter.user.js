// ==UserScript==
// @name         WME Road Shield Helper - ABelter fork
// @namespace    https://github.com/abelter/
// @version      2023.02.21.01
...
(function() {
    ...
    let TESTERS = ["The_Cre8r","jm6087","s18slider","locojd1","SethSpeedy28","nzahn1","Harmonious4","turnertr","sketch","phuz","ABelter"];
...
//        if (TESTERS.indexOf(W.loginManager.user.userName) > -1) {
            ...
            document.querySelector("#WMERSH-TIO-Autofill").onclick = function(){
                ...
                Strings.forEach(function(item, index){
                    ...
                    let subDirection = false;
                    let baseItem = item.replace(/(?: (N|E|S|W))$/,'');
                    let baseDirection;
                    switch (match2[4]) {
                        case "N":
                            baseDirection = "Nᴏʀᴛʜ" //North
                            break;
                        case "E":
                            baseDirection = "Eᴀꜱᴛ" //East
                            break;
                        case "S":
                            baseDirection = "Sᴏᴜᴛʜ" //South
                            break;
                        case "W":
                            baseDirection = "Wᴇꜱᴛ" //West
                            break;
                        default:
                            baseDirection = ""
                            break;
                    ...
                    if (match2[1] && match2[2]) {
                        ...
                        if (shieldID > -1) {
                            ...
                            if (subDirection) {
                                document.querySelector(`#panel-container > div > div > div.panel-content > div:nth-child(1) > div > div > div > span:nth-child(${index+adjIndex+1}) > span > input#direction`).value = baseDirection;
                            }
